...
def load_dataset(
    ...
    images_dir: str,
    scribbles_dir: str,
    ground_truth_dir: Optional[str] = None  # Use Optional instead of |
...
def calculate_iou_metrics(predictions: np.ndarray, ground_truth: np.ndarray) -> dict:
    """
    Calculate IoU metrics for binary segmentation.
    
    Args:
        predictions (np.ndarray): Array of shape (N, H, W) with predicted class labels (0 or 1).
        ground_truth (np.ndarray): Array of shape (N, H, W) with ground truth class labels (0 or 1).
        
    Returns:
        dict: Dictionary containing Mean IoU, Background IoU, and Object IoU
    """
    # Ensure arrays are binary (0 or 1)
    ...
    for pred, gt in zip(predictions, ground_truth):
        ...
        bg_intersection = np.sum((pred == 0) & (gt == 0))
        bg_union = np.sum((pred == 0) | (gt == 0))
        bg_iou = bg_intersection / bg_union if bg_union > 0 else 1.0
        background_ious.append(bg_iou)
        
        # Calculate IoU for object/foreground (class 1)
        obj_intersection = np.sum((pred == 1) & (gt == 1))
        obj_union = np.sum((pred == 1) | (gt == 1))
        obj_iou = obj_intersection / obj_union if obj_union > 0 else 1.0
        object_ious.append(obj_iou)
    
    # Calculate mean IoU values
    mean_bg_iou = np.mean(background_ious)
    mean_obj_iou = np.mean(object_ious)
    mean_iou = (mean_bg_iou + mean_obj_iou) / 2
    
    return {
        'Mean IoU': mean_iou,
        'Background IoU': mean_bg_iou,
        'Object IoU': mean_obj_iou
    }

def print_iou_metrics(predictions: np.ndarray, ground_truth: np.ndarray, team_name: str = "Your Team"):
    """
    Calculate and print IoU metrics in a formatted table.
    
    Args:
        predictions (np.ndarray): Array of shape (N, H, W) with predicted class labels.
        ground_truth (np.ndarray): Array of shape (N, H, W) with ground truth class labels.
        team_name (str): Name of your team for display.
    """
    metrics = calculate_iou_metrics(predictions, ground_truth)
    
    print(f"\nIoU Metrics for {team_name}:")
    print("="*50)
    print(f"{'Team':<15} {'Mean IoU':<12} {'Background IoU':<15} {'Object IoU':<12}")
    print("-"*50)
    print(f"{team_name:<15} {metrics['Mean IoU']:<12.4f} {metrics['Background IoU']:<15.4f} {metrics['Object IoU']:<12.4f}")
    print("="*50)
